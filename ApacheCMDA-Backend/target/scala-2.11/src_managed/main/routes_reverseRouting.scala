...
// @LINE:79
def addFollowee(followerID:Long, followeeID:Long): Call = {
   import ReverseRouteContext.empty
   Call("POST", _prefix + { _defaultPrefix } + "users/" + implicitly[PathBindable[Long]].unbind("followerID", followerID) + "/follow/" + implicitly[PathBindable[Long]].unbind("followeeID", followeeID))
}
...
// @LINE:79
def addFollowee : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.UserController.addFollowee",
   """
      function(followerID,followeeID) {
      return _wA({method:"POST", url:"""" + _prefix + { _defaultPrefix } + """" + "users/" + (""" + implicitly[PathBindable[Long]].javascriptUnbind + """)("followerID", followerID) + "/follow/" + (""" + implicitly[PathBindable[Long]].javascriptUnbind + """)("followeeID", followeeID)})
      }
   """
)
...
// @LINE:79
def addFollowee(followerID:Long, followeeID:Long): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).addFollowee(followerID, followeeID), HandlerDef(this.getClass.getClassLoader, "", "controllers.UserController", "addFollowee", Seq(classOf[Long], classOf[Long]), "POST", """""", _prefix + """users/$followerID<[^/]+>/follow/$followeeID<[^/]+>""")
)
